...
const getMatrixProjects = async () => {
    try {
        ...
        const insertQuery = 'INSERT INTO projectname (ProjectName) VALUES (?)';
        for (const projectName of projectNames) {
            try {
                await db.promise().execute(insertQuery, [projectName]);
            } catch (error) {
                console.error(`Error inserting project name "${projectName}":`, error);
            }
        }
