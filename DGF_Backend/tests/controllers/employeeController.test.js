...
describe("getEmployeesByDesignationController", () => {
  ...
  test("should return 400 if designationNames is missing", async () => {
    req.query = {}; // Empty query

    await getEmployeesByDesignationController(req, res);

    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ error: "Designation names are required" });
  });
