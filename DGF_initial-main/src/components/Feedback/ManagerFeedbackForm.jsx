...
const ManagerFeedbackForm = () => {
  ...
  const [course_id, setCourseId] = useState("");
  const [employee_id, setEmployeeId] = useState("");
  const [manager_id, setManagerId] = useState(""); // Manager ID state
  const [dialogOpen, setDialogOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const courseIdParam = urlParams.get("course_id");
    const employeeIdParam = urlParams.get("employee_id");
    const managerIdParam = urlParams.get("manager_id"); // Extract manager_id

    setReqid(reqidParam);
    setCourseId(courseIdParam);
    setEmployeeId(employeeIdParam);
    setManagerId(managerIdParam); // Set manager_id

    // Fetch data from the new backend API
    const fetchData = async () => {
      try {
        if (!reqidParam || !courseIdParam || !employeeIdParam || !managerIdParam) {
          console.error("Missing required query parameters: reqid, course_id, employee_id, or manager_id.");
          alert("Invalid request. Missing required parameters.");
          return;
        }

        const response = await axios.get(`${backendUrl}api/effectiveness-feedback/feedback/details`, {
          params: { reqid: reqidParam, course_id: courseIdParam, employee_id: employeeIdParam, manager_id: managerIdParam },
        });
  ...
  const requestData = {
    ...
    course_id,
    employee_id,
    manager_id,  // Ensure manager_id is included
    demonstrate_skill: formData.demonstrateSkill,
    skill_date: formData.skillDate || null,
    enhancement_rating: formData.enhancementRating || null,
    suggestions: formData.suggestions,
    opportunity_date: formData.opportunityDate || null,
  };

  try {
    const response = await axios.post(`${backendUrl}api/manager-feedback`, requestData);
    ...
    // Handle error specifically for feedback submission window
    if (error.response && error.response.data.message === "You have already submitted feedback for this course within the past 6 months.") {
      alert("You have already submitted feedback for this course. You can submit it again after 6 months.");
    } else {
