...
import {  Paper,  Typography,  Select,  MenuItem,  FormControl,  RadioGroup,
  TextField,  FormControlLabel,  Radio,  Button,  Box,  Divider,  TableCell,
  TableContainer,  TableBody,  TableHead,  TableRow,  Table,  IconButton,
  Avatar,  Autocomplete,Dialog, DialogTitle,} from "@mui/material";
...
const NewTrainingRequest = () => {
  ...
  const handleEmailSearch = async (email) => {
    try {
      const response = await fetch(`http://localhost:8000/api/employee/search-by-email?email=${email}`);
      const data = await response.json();
  ...
  // const updateAllEmployees = (field, value) => {
  //   setFormData(prevFormData => ({
  //     ...prevFormData,
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      ...
      >
        ...
        >
          ...
          <Typography style={{ marginLeft: "-0.85rem" }}>
          <strong> <span>{user ? toPascalCase(user.name) : "User"}</span></strong>
          </Typography>
        </Box>
        <Paper
          elevation={1}
          ...
          >
           ...
           />
            ...
            <FormControl fullWidth className="formControl">
              <Typography
                className="subheader"
                style={{ display: "inline", marginBottom: "0.5rem" , marginLeft:'0rem', marginRight:'0.8rem'}}
              >
    ...
    >
      {source.source_name}
            ...
            <FormControl fullWidth className="formControl">
              ...
              >
                Training Objective <span className="required">*</span>
              </Typography>
              <Select
  variant="outlined"
      ...
      {objective.training_name}
          ...
          <FormControl component="fieldset" style={{ marginBottom: "0.5rem" }}>
            ...
            >
              <FormControlLabel
                value="prospect"
                control={<CustomRadio />}
                label={<Typography className="subheader" style={{marginLeft:'-6px'}}>Prospect</Typography>}
              />
              <FormControlLabel
                value="project"
                control={<CustomRadio />}
                label={<Typography className="subheader" style={{marginLeft:'-6px'}}>Project</Typography>}
              />
            </RadioGroup>
          </FormControl>
          {formData.trainingPurpose === "prospect" ? (
            <Box
              ...
              flexWrap='wrap'
            >
              <FormControl fullWidth className="formControl" style={{
      flexBasis: '30%',
      minWidth: '250px',
      display: 'flex',
...
>
  <MenuItem
    disabled
    value=""
    style={{ fontSize: "12px", padding: "4px 4px 4px 6px", fontFamily:'Poppins' }}
  >
    <em style={{ height: "30px", opacity: "0.75" , fontFamily:'Poppins'}}>
      Select Project
    </em>
  </MenuItem>
          ...
          >
            ...
            >
              <FormControlLabel
                ...
                control={<CustomRadio />}
                label={
                  <Typography className="subheader" style={{marginLeft:'-6px'}}>Add Employees</Typography>
                }
              />
              <FormControlLabel
                value="open"
                control={<CustomRadio />}
                label={
                  <Typography className="subheader" style={{marginLeft:'-6px'}}>
          ...
          <Box style={{ width: "100%" }}>
           ...
           />
                  ...
                  <Typography
                    className="subheader"
                    align="center"
                    style={{ margin: " 30px" , marginLeft:'10px', marginRight:'10px'}}
                  >
                    OR
                  </Typography>
                  <FormControl fullWidth className="formControl">
    ...
    <Table size="smaller">
      <TableHead className="head" >
        ...
        <TableRow>
          ...
          <TableCell></TableCell>
          <TableCell>
            <TextField className="availabledates"
              type="date"
              onChange={(e) => setFormData({ ...formData, availableFrom: e.target.value })}
              
              sx={{ fontSize: '10px', fontFamily: 'Poppins' , letterSpacing: '-0.5px',   // Reduce letter spacing
                padding: '5px', }}
            />
          </TableCell>
          <TableCell>
            ...
            style={{fontSize:'12px'}}
              onChange={(e) => setFormData({ ...formData, updateAllBandwidth: e.target.value })}
              size="small"
              sx={{
            ...
            >
              <MenuItem value="" sx={{ fontSize: '10px', fontFamily: 'Poppins' }}>Select</MenuItem>
              <MenuItem value="2 Hours" sx={{ fontSize: '10px', fontFamily: 'Poppins' }}>2 Hours</MenuItem>
              <MenuItem value="4 Hours" sx={{ fontSize: '10px', fontFamily: 'Poppins' }}>4 Hours</MenuItem>
              <MenuItem value="6 Hours" sx={{ fontSize: '10px', fontFamily: 'Poppins' }}>6 Hours</MenuItem>
              <MenuItem value="Full Day" sx={{ fontSize: '10px', fontFamily: 'Poppins' }}>Full Day</MenuItem>
            </Select>
          </TableCell>
          <TableCell>
            <RadioGroup row  onChange={(e) => setFormData({ ...formData, updateAllWeekend: e.target.value })}>
              <FormControlLabel value="Yes" control={<Radio size="small" />} label="Yes" />
              <FormControlLabel value="No" control={<Radio size="small" />} label="No" />
            </RadioGroup>
          </TableCell>
          <TableCell>
            <Button variant="contained" style={{textTransform:'none', padding:'5px'}} onClick={() => updateAllEmployees({ availableFrom: formData.availableFrom, bandwidth: formData.updateAllBandwidth, weekend: formData.updateAllWeekend })} size="small">
              Update All
      ...
      <TableBody>
        {formData.employees.slice(formData.page * formData.rowsPerPage, formData.page * formData.rowsPerPage + formData.rowsPerPage).map((employee) => (
          <TableRow key={employee.uniqueKey}>
            ...
            <TableCell>
              <Select value={employee.bandwidth} onChange={(e) => updateEmployee(employee.id, "bandwidth", e.target.value)} size="small"  sx={{
...
/>
  ...
  onChange={(e) =>
    setFormData({
      ...formData,
      selectedEmployeeLevel: e.target.value === formData.selectedEmployeeLevel ? "" : e.target.value,
    })
  }
...
>
  ...
  onChange={(e) =>
    setFormData({
      ...formData,
      selectedPrimarySkill: e.target.value === formData.selectedPrimarySkill ? "" : e.target.value,
    })
  }
  style={{
    height: "30px",
            ...
            >
              ...
              <Box style={{ height: "auto", width: "100%" }}>
                <ReactQuill
                  ...
                  onChange={(value) =>
                    setFormData({ ...formData, otherSkill: value })
                  }
                  modules={{
...
>
  <DialogTitle style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '24px' }}>
    <CheckCircleIcon style={{ color: 'green', fontSize: '3rem', marginBottom: '10px' }} />
    Request Sent Successfully with the request number : <strong style={{ fontSize: '18px', color: '#333' }}>{newRequestId}</strong>.
    <IconButton onClick={handleCloseDialog} style={{ position: 'absolute', right: '10px', top: '10px', padding: '0' }}>
